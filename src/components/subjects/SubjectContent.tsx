          ...
          <div className="p-4 space-y-4">
            {subjects.map((subject) => (
              <div key={subject.id} className="space-y-2">
                <Button
                  variant="ghost"
                  className={`w-full justify-start font-medium ${subject.id === currentSubject?.parent_id || subject.id === currentSubject?.id ? "bg-gray-100" : ""}`}
                    ...
                    <div className="ml-4 space-y-1 border-l border-gray-200">
                      {subject.children.map((child) => (
                        <Button
                          key={child.id}
                          variant="ghost"
                          className={`w-full justify-start pl-4 ${child.id === currentSubject?.id ? "bg-gray-100" : ""}`}
                ...
                <div className="aspect-video relative">
                  <img
                    src={
                      ...
                      `https://source.unsplash.com/random/800x600/?learning,${encodeURIComponent(course.title)}`
                    }
                    alt={course.title}
